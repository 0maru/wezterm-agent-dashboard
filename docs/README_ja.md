...
- `TaskCreate` / `TaskUpdate` に基づく Task/Todo 進捗表示
- トークン使用量・コストの追跡
...
Task 進捗は TaskCreate / TaskUpdate 系イベントと TodoWrite の snapshot から復元します。hook payload に error response が含まれる場合、失敗情報も activity log の label に残します。
